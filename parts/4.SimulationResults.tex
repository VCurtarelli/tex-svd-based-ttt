...
\def\sympathlayer{test-bas-vs-svd-layer-260422-13h24m19s}
\def\sympathsurround{test-bas-vs-svd-surround-260422-13h35m50s}
\def\sympath{\sympathlayer}
\input{figures/data/\sympath/params.tex}

To compare in practice the two techniques presented, simulated SSFs were created, in which these techniques were employed for the inversion step of estimating the SSF. Namely, we will compare the literature standard technique from \cref{eq:sec2:minim_solution_literature}, with the proposed method as obtained in \cref{eq:sec3:minim_solution_proposed}. In this section, we present the environment and practical conditions assumed, detail the considerations taken for the exhibited techniques, as well as other underlying necessary parameters, methods, and metrics; and finally, we bring forth the results, and their following discussions.
...
We also consider the sound speed field to follow the formula derived by \citeauthor{delgrosso_new_1974}, with empirical functions for the temperature and salinity following the thermocline and halocline \cite{janecki_new_2022}. The considered SSF is presented in \cref{subfig:sec4:discretized_ssf_with_profiles:fig1}, with its profiles --- each 1D slice for a fixed $\x$-axis offset --- in \cref{subfig:sec4:discretized_ssf_with_profiles:fig2}.

Along with the SSF, we also consider two distributions of devices, presented in \cref{subfig:sec4:discretized_ssf_with_profiles:fig3,subfig:sec4:discretized_ssf_with_profiles:fig4}: in the first case, the sources are along the bottom and right-side, with receivers along the surface and left-side; and in the second case, the sources are strictly along the bottom, and receivers only on the surface. In both cases and figures, the sources are represented by blue circles, and receivers by red exes.

% \Cref{subfig:sec4:discretized_ssf_with_profiles:fig1} also shows the chosen positions for the sources (in blue with $\bigcirc$, along the floor) and receivers (in red with $\times$, along the surface). This device positioning aligns with the practical underlying motivation, where ocean bottom nodes (OBNs) \cite{obns} are proposed to be used both as receivers for sub-surface imaging, and sources for the above-ground oceanic environment modeling; the later being necessary for a faithful reconstruction of the subterranean structures \cite{?}.
