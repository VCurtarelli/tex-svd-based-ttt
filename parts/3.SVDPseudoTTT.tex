...
We denote $\bvSi[t]$ as a truncated $\bvSi$, with only its first $K$ diagonal entries being non-zero. Trivially, $K \leq R$ for this to be meaningful. Given this approximation, we define $\bvR[t]{i} = \bvU \bvSi[t] \bvV[T]$, and the inverse map function can be approximated as
\begin{equation}
	\begin{split}
		\bvG{i}
		& = \pinv{\bvR[t]{i}} \\
...
Notably, $K = R$ degenerates to $\bvR[t]{i} = \bvR{i}$. For the truncated solution in \cref{eq:sec3:Gi_truncated}, $\nabla E(\bvs{i+1}) \neq \bv0$. Instead, we can show that (\cref{thm:svd-truncated_non-zero-gradient}, \cref{app:appB:proofs})
\begin{equation}
	\nabla E(\bvs{i+1}) = 2 \pts{ \sum_{j=K+1}^{R} \lambda_j \bvv{j} \bvu[T]{j}} \bvt{\obs},
\end{equation}
...
With this, both the cost function and its gradient are unaffected by $\bvmu$, for any $\bvmu$ and $\bvG{i}$. This generalizes the singular solution obtained -- from \cref{eq:sec3:Gi_proposed} or \cref{eq:sec3:Gi_truncated} -- to a solution-space that spans the forward map's null-space, within which the primary cost function $E(\bvs)$ is invariant.

For the validity of \cref{eqs:sec3:cost-gradient_null-space}, $\bvmu$ must be a coordinate vector for the null-space of $\bvR{i}$, not of $\bvR[t]{i}$. If $\bvV{2}$ comprises the last $N-K$ columns of $\bvV$, then the equations in \cref{eqs:sec3:cost-gradient_null-space} aren't valid, with there being some truncation-related residual cost function; more so, in this case the gradient will be (\cref{thm:diff_gradient_null-space-exploiting_truncated}, \cref{app:appB:proofs})
\begin{equation}
	\nabla E( \bvG{i} \bvt{\obs} + \bvV[t]{2} \bvmu) = \nabla E(\bvG{i} \bvt{\obs}) + \sum_{j=K+1}^{R} \lambda_j^2 \mu_{j-K} \bvv{j},
\end{equation}
in which $\mu_{m}$ is the $m$-th element of $\bvmu$. From this, only the first $R-K$ entries of $\bvmu$ -- the ones relative to the truncated singular values -- interfere in the gradient, modulated by the respective singular value squared. That is, the smaller the truncated singular values are, the lesser their impact on the difference in gradient will be.

Through the rest of the text, we assume that $\bvV{2}$ forms the basis for the null-space of $\bvR[t]{i}$, allowing the presence of vestigial differences between the achieved and the true minimum.

\subsection{Secondary minimization}

This information on the independence of $\bvmu$ and null-space can be leveraged to find an optimal solution within a secondary metric, while minimally affecting the cost function and its gradient. Given a regularization matrix $\bvD$ as before, we define a new cost function $F(\bvG{i} \bvt{\obs},\bvmu)$ to be minimized w.r.t. $\bvmu$. We let $\bvmu{i+1}$ be the ($i$+1)-th iteration's optimum $\bvmu$, defined as
\begin{equation}
	\bvmu{i+1} = \argmin_{\bvmu} F(\bvG{i} \bvt{\obs},\bvmu) = \norm{\bvD\pts{\bvG{i} \bvt{\obs} + \bvV{2} \bvmu}}^2.
\end{equation}

Since by definition $\bvV{2}$ is a column-orthogonal matrix (meaning $\rank{\bvV{2}} = N-K$), if $\bvD$ is a full-rank matrix then the solution is given by
\begin{equation}
...
\end{equation}
where $\bvG{i}$ is from \cref{eq:sec3:Gi_truncated}.
...
An important remark is that $\bvV{2} \pinv*{\bvD \bvV{2}} \bvD$ can't be further simplified. By the p-inverse's properties, this would require $\bvV{2}$ to have linearly independent rows, which it can't given it is a tall matrix ($N > N-K$). %Another relevant attribute is that, if $\bvD$ is invertible, the solution for taking the gradient of $F(\bvG{i} \bvt{\obs},\bvmu)$ w.r.t. $\bvmu$, $\bvG{i}\bvt{\obs}$, or $\bvG{i}\bvt{\obs}+\bvV{2}\bvmu$, all result in \cref{eq:sec3:bvmu_solution}.

\subsection{Iterative model tuning}

It is easy to see that a smaller $K$ implies a $\bvV{2}$ matrix with more columns, increasing the practical null-space of $\bvR{i}$, and thus the effectiveness of the secondary (regularizing) minimization; however, increasing $K$ increases the primary (travel-time error) cost function and gradient errors. Therefore, the parameter $K$ controls the tradeoff between the primary and secondary cost functions.

One possible exploitable venue is to increase $K$ over the iterations, or when the minimization reached a plateau. Since each iteration will move the solution closer to the optimum within the considered null-space, decreasing this null-space will allow us to achieve a better solution overall, improving on the previous iterations results.
