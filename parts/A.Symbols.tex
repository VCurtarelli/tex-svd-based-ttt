\section*{List of symbols}
\label{app:appA:list_of_symbols}
Here we present a comprehensible list of the most relevant symbols and variables used throughout the paper. We note that non-bold variables are scalars, bold lowercase variables are vectors, and bold uppercase variables are matrices.

\noindent\begin{tabular}{l l}
	$M$ & Number of rays \\
	$N$ & Number of cells \\
	$\bvs$ & Slowness model vector \\
	$\bvc$ & Speed model vector \\
	$\bvR$ & Travel-distance for all rays \\
	$\bvt{\obs}$ & Observed travel-times \\
	$\bvD$ & Regularization function \\
	$\alpha$ & Regularization factor \\
	$\bvG$ & Inverse map function \\
	$\bvU$ & Left orthonormal base of $\bvR$ \\
	$\bvV$ & Right orthonormal base of $\bvR$ \\
	$\bvSi$ & Singular values diagonal matrix \\
	$\bvSi[t]$ & Truncated singular values diagonal matrix \\
	$\bvU{2}$ & $\bvR$'s left-null-space orthonormal basis \\
	$\bvV{2}$ & $\bvR$'s right-null-space orthonormal basis \\
	$\bvmu$ & Right-null-space coordinate vector
%	$\bvy[l,k]$ & Observed signal vector \\
%	$\bveta[l,k]$ & Global noise vector \\
%	$z_m[l,k]$ & Token signal $z$ at $m$-th sensor\\
%	$\bvz[l,k]$ & Token signal $z$ vector \\
%	$\Corr{\bvz}[k]$ & Correlation matrix for $\bvz$\\
%	$\Corr*{\bvy}[k]$ & Observed signal estimate CM\\
%	$\var{z}[k]$ & Variance of the signal $z$ \\
%	$\theta_b$ & Undesired source's azimuth \\
%	$\phi_b$ & Undesired source's elevation \\
%	$\Theta = (\theta_b,\phi_b)$ & Undesired source's DoA \\
%	$\bvsi[k]$ & Variance vector \\
%	$\bvSi$ & Cross-frequency variance matrix \\
%	$\cost{\bvSi,\Theta}$ & Global cost function for optimization \\
%	$\bvze[k]$ & Lagrange multiplier	\\
%	$\bvh[k]$ & Beamformer vector \\
%	$\epsilon$ & Regularization factor \\
%	$C[k]$ & Constraint matrix for LCMV beamformer
\end{tabular}