%% Requires:
% PGFPlots

%BEGIN_FOLD
\usepgfplotslibrary{external}
\usepgfplotslibrary{polar}
\newlength{\Yaxisshift}
\setlength{\Yaxisshift}{3.8cm}

\usetikzlibrary{external}
% \tikzexternalize
% \tikzsetexternalprefix{/figures/tikz/}
\tikzexternalize[prefix=figures/tikzexternal/,figure name=output]

\NewDocumentEnvironment{polar}{}{
	\begin{polaraxis}[
		%BEGIN_FOLD
		xtick = {
			0,
			30,
			60,
			90,
			120,
			150,
			180,
			210,
			240,
			270,
			300,
			330
		},
		xticklabels = {
			$0\dg$,
			$30\dg$,
			$60\dg$,
			$90\dg$,
			$120\dg$,
			$150\dg$,
			$\PM180\dg$,
			$-150\dg$,
			$-120\dg$,
			$-90\dg$,
			$-60\dg$,
			$-30\dg$
		},
		ymax=0, ymin=\ymin,
		rotate=-90,
		width=0.75\linewidth,
		height=0.75\linewidth,
		x dir=reverse,
		xticklabel style={anchor=-\tick-90},
		y coord trafo/.code=\pgfmathparse{##1-\ymin},
		y coord inv trafo/.code=\pgfmathparse{##1\ymin},
		yticklabel={$\pgfmathprintnumber{\tick}\dB$},
		yticklabel style={anchor=west, xshift=\Yaxisshift+1ex},
		y axis line style={yshift=\Yaxisshift},
		ytick style={yshift=\Yaxisshift},
		ytick align=outside,
		ytick = {0, -20, ..., \ymin},
		ylabel={$\abs{\beam}$ (dB)},
		ylabel style={xshift=\Yaxisshift, yshift=0.16\linewidth},
		%END_FOLD
	]
}{
	\end{polaraxis}
}
%END_FOLD

%BEGIN_FOLD
\usepgfplotslibrary{colorbrewer}
\NewDocumentEnvironment{heatmap}{m O{} O{}}{
	\begin{axis}[
		colormap/viridis,
		colorbar horizontal,
		colorbar style={
			colormap/viridis,
			width=0.75\linewidth,
			height=12pt,
			%			xlabel = Beampattern,
			xtick = {0.5*(\zmin+\zmax)},
			xticklabel style={
				/pgf/number format/.cd,
				fixed,
				%title=,
				precision=0,
				fixed zerofill,
				%				opacity=1
			},
			tick label style={opacity=1},
			major tick length=4pt,
			xtick style={black},
			xtick pos=top,
			extra x ticks = {\zmin,\zmax},
			extra x tick style = {major x tick style={draw=none}},
			point meta min=\zmin,
			point meta max=\zmax,
			#3
		},
		width=0.9\linewidth,
		height=0.9\linewidth,
		point meta min=\zmin,
		point meta max=\zmax,
		mesh/cols=#1,
		xlabel style = {anchor=north, yshift=0.9\linewidth-2.5em},
		view={0}{90},
		grid style = {draw=none},
		ylabel style={anchor=center, yshift=-0.9\linewidth+2.5em},
		major tick length=2pt,
		ytick style = {black},
		ytick pos = left,
		mesh/ordering=x varies,
		y dir=reverse,
		#2
		]
		\centering\footnotesize
	}{
	\end{axis}
}
%END_FOLD

\newlength{\wdtwenty}
\setlength{\wdtwenty}{\widthof{$-20$}}
%\setlength{\wdtwenty}{\widthof{-20}}
\pgfplotsset{
    every colorbar global/.append style={
        zmin=,zmax=,
    }
}

%BEGIN_FOLD
\NewDocumentEnvironment{lineplot}{O{axis} m m O{}}{ % xlabel | ylabel | other arguments
	\begin{#1}[
		width=0.8\linewidth,
		height=0.65\linewidth,
		%
		%		xtick = {1, 2 ,..., 7},
		xtick align=outside,
		xtick pos=bottom,
		ytick align=outside,
		ytick pos=left,
		%
		xlabel = #2,
		ylabel = #3,
		ylabel near ticks,
		%		yticklabel style={text width=\wdtwenty, align=right},
		ylabel style={yshift=-3pt},
		% xlabel style={yshift=0.5em},
		%
		%
		%		legend pos={outer south},
		%
		legend cell align={left},
		legend style={
			fill=white,
			%			fill opacity=0.5.6,
			draw opacity=1,
			text opacity=1,
			%			at={(1.05, 0.5)},
			%			anchor=west,
			legend columns=2,
			/tikz/every even column/.append style={column sep=1em},
			font=\footnotesize
		},
		%		yticklabel pos=right,
		%		yticklabel style={
			%			text width=2em,
			%			anchor=west,
			%			align=left,
			%			% xshift=-0.2em,
			%		},
		scale only axis,
		#4,
		]
		\centering\footnotesize
		%		\draw[thin, dash pattern = {on 4pt off 1pt}] (axis cs:\pgfkeysvalueof{/pgfplots/xmin},0) -- (axis cs:\pgfkeysvalueof{/pgfplots/xmax},0);
	}{
	\end{#1}
}
%END_FOLD

%BEGIN_FOLD
\tikzstyle{axis} = [
	black
]
\tikzstyle{resA} = [
	thick, ColA, dash pattern = {on 1pt off 0pt},
	mark=square*, mark size=1, mark options={
		rotate=0,
		solid,
	}
]
\tikzstyle{resB} = [
	thick, ColB, dash pattern = {on 2pt off 1pt},
	mark=triangle*, mark size=1.5, mark options={
		rotate=0,
		solid,
	}
]
\tikzstyle{resC} = [
	thick, ColC, dash pattern = {on 2pt off 2pt},
	mark=triangle*, mark size=1.5, mark options={
		rotate=180,
		solid,
	}
]
\tikzstyle{resD} = [
	thick, ColD, dash pattern = {on 2pt off 3pt},
	mark=square*, mark size=1.2, mark options={
		rotate=45,
		solid,
	}
]
\tikzstyle{resE} = [
	thick, ColE, dash pattern = {on 3pt off 1pt},
		mark=star, mark size=2, mark options={
		rotate=0,
		solid,
		very thick
	}
]
\tikzstyle{resF} = [
	thick, ColF, dash pattern = {on 3pt off 2pt},
	mark=x, mark size=2, mark options={
		rotate=0,
		solid,
		very thick
	}
]
\tikzstyle{resG} = [
	thick, ColG, dash pattern = {on 3pt off 4pt},
	mark=+, mark size=2, mark options={
		rotate=0,
		solid,
		very thick
	}
]
%END_FOLD

\pgfplotsset{
	hmstyle/.style={
		ytick={0, 500, ..., 2000},
		yticklabels={0, 0.5, ..., 2},
		xtick={0, 1000, 2000},
		xticklabels={0, 1, 2},
		xmin=0, xmax=2000,
		ymin=0, ymax=2000,
		xlabel={Offset ($\si{\kilo\meter}$)},
		ylabel={Depth ($\si{\kilo\meter}$)},
	}
}
\pgfplotsset{
	profilestyle/.style={
		y dir=reverse,
		ytick={0, 500, ..., 2000},
		yticklabels={0, 0.5, ..., 2},
		xtick={1490, 1500, ..., 1550},
		xticklabels={$1.49$, $1.5$, $1.51$, $1.52$, $1.53$, $1.54$, $1.55$},
		xmin=1489, xmax=1547,
		ymin=0, ymax=2000,
		xlabel={Sound speed ($\si{\kilo\meter/\second}$)},
		ylabel={Depth ($\si{\kilo\meter}$)},
		yticklabel style={anchor=north east}
%		scale only axis
	}
}

\NewDocumentCommand{\colorbar}{m}{%
	\tikzexternaldisable \ref*{#1} \tikzexternalenable%
}